...
class OrderController extends Controller
{
        ...
        foreach ($carts as $c) {
            ...
            if ($product) {
                ...
                $or_d = [
                    ...
                    'item_note' => $c['item_note'] ?? 'any value of code',
                    'created_at' => now(),
                    'updated_at' => now()
                ];
                $order_details[] = $or_d;
    ...
    public function get_order_list(Request $request)
    {
        ...
        $fortyMinutesAgo = Carbon::now()->subMinutes(40);
        $user_id = $request->user ? $request->user->id : $request['guest_id'];

        $paginator = Order::with(['restaurant', 'delivery_man.rating'])->withCount('details')->where(['user_id' => $user_id])->
        whereIn('order_status', ['delivered','canceled','refund_requested','refund_request_canceled','refunded','failed','pending','confirmed' ])
        ->where('created_at', '<=', $fortyMinutesAgo)
        ->Notpos()
    ...
    public static function  coupon_check($request){
        ...
        if (isset($coupon)) {
            ...
            return ['coupon' => $coupon ,'coupon_created_by' => $coupon_created_by ,'delivery_charge' => $delivery_charge??0 , 'free_delivery_by' => $free_delivery_by??null  ];
        } else {
...
public function get_takeaway_time_slots(Request $request)
{
    try {
        // Validate and get the date from request
        $request->validate([
            'date' => 'required|date_format:Y-m-d'
        ]);
        
        $dateInput = $request->input('date');
        $selectedDate = Carbon::parse($dateInput, 'Europe/Dublin');
        $today = Carbon::today('Europe/Dublin');

        // Set timezone to Dublin
        $now = Carbon::now('Europe/Dublin');
        
        // Fixed opening hours (5 PM to 10 PM)
        $opening_time = Carbon::createFromTime(17, 0, 0, 'Europe/Dublin');
        $closing_time = Carbon::createFromTime(22, 0, 0, 'Europe/Dublin');
        
        // Determine if selected date is today
        $isToday = $selectedDate->isSameDay($today);

        // Calculate first slot based on conditions
        if ($isToday) {
            // For today - check current time
            if ($now < $opening_time) {
                $first_slot = $opening_time->copy()->addMinutes(25);
            } else {
                $first_slot = $now->copy()->addMinutes(25);
            }
        } else {
            // For future dates - always start at 5:25 PM
            $first_slot = $selectedDate->copy()
                ->setTime(17, 0, 0) // 5 PM
                ->addMinutes(25);
        }

        // Round up to next 5 minute interval
        $minutes = $first_slot->minute;
        if ($minutes % 5 !== 0) {
            $first_slot->addMinutes(5 - ($minutes % 5));
        }

        // For future dates, make sure we're using that date's opening time
        if (!$isToday) {
            $opening_time = $selectedDate->copy()->setTime(17, 0, 0);
            $closing_time = $selectedDate->copy()->setTime(22, 0, 0);
        }

        // Make sure first slot is within opening hours
        if ($first_slot < $opening_time) {
            $first_slot = $opening_time->copy()->addMinutes(25);
        }

        // Generate slots in 12-hour format
        $slots = [];
        $current = $first_slot->copy();
        
        while ($current <= $closing_time) {
            $slots[] = $current->format('g:i A'); // 12-hour format with AM/PM
        ...
        return response()->json([
            ...
            'is_today' => $isToday,
            'slots' => $slots,
            'next_slot_in_minutes' => $isToday ? $now->diffInMinutes($first_slot) : null,
            'first_slot' => $first_slot->format('g:i A')
        ]);
...
public function get_delivery_time_slots(Request $request)
{
    try {
        // Validate and get the date from request
        $request->validate([
        ...
        if ($isToday) {
            // For today - check current time
            if ($now < $opening_time) {
                $first_slot = $opening_time->copy()->addMinutes(45);
            } else {
                $first_slot = $now->copy()->addMinutes(45);
            }
        } else {
            // For future dates - always start at 5:25 PM
            $first_slot = $selectedDate->copy()
