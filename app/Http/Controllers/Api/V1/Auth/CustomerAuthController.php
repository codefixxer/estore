...
class CustomerAuthController extends Controller
{
    ...
    // web base delete user for play store condition 
public function delete_account(Request $request)
{
    ...
    // Verify password - moved before try-catch since it's a validation check
    if (!Hash::check($request->password, $user->password)) {
        return response()->json([
            'message' => translate('Email or password does not match')
        ], 403);
    }

    try {
        // For testing purposes, we'll just log the deletion instead of actually deleting
        \Log::info('Account deletion requested for user: ' . $user->email);
        \Log::info('User ID: ' . $user->id);
        \Log::info('User Name: ' . $user->f_name . ' ' . $user->l_name);
