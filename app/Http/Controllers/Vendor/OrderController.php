...
public function list($status, Request $request)
{
    ...
    // Determine self-delivery capability
    ...
    // Mark new orders as checked
    Order::where('checked', 0)
        ->where('restaurant_id', Helpers::get_restaurant_id())
        ->update(['checked' => 1]);
    ...
    $query = Order::with('customer')
        ...
        ->when($status === 'all', function($q) use ($selfDelivery) {
            return $q->where(function($sub) use ($selfDelivery) {
                ...
                $sub->whereNotIn('order_status',$excluded)
                    ->orWhere(fn($q2)=> $q2->where('order_status','pending')
                                           ->whereIn('order_type',['take_away','dine_in']))
                    ->orWhere(fn($q3)=> $q3->where('order_status','pending')
        ...
        ->when(count($key)>0, function($q) use ($key) {
            return $q->where(fn($q2)=> collect($key)
                ->each(fn($word)=> $q2->orWhere('id','like',"%{$word}%")
    ...
    $counts = (object)[
        'all'       => Order::where('restaurant_id',Helpers::get_restaurant_id())
                            ...
                            ->count(),
        'pending'   => Order::where('restaurant_id',Helpers::get_restaurant_id())
                            ->where('order_status','pending')
                            ->Notpos()->hasSubscriptionToday()
                            ->OrderScheduledIn(30)
                            ->count(),
        'confirmed' => Order::where('restaurant_id',Helpers::get_restaurant_id())
                            ->whereIn('order_status',['confirmed','delivered','cooking','handover'])
                            ->whereNotNull('confirmed')
                            ->Notpos()->hasSubscriptionToday()
                            ->OrderScheduledIn(30)
                            ->count(),
    ...
    //         });
    //     })
    //      ->when(!$request->has('filter_date'), function ($query) {
    //         // Default to today's date if no filter_date is provided
    //         return $query->whereDate('created_at', Carbon::today());
    //     })
    //     ->when($request->has('filter_date'), function ($query) use ($request) {
...
{
    // 1) Validate only id & order_status
    $request->validate([
        'id'           => 'required',
    ...
    $order = Order::where([
        ...
        ->with(['subscription_logs','details'])
        ->first();
    ...
    // 3) Prevent status changes after delivered
    if ($order->delivered != null) {
        Toastr::warning(translate('messages.cannot_change_status_after_delivered'));
        return back();
    }

    // 4) Cancel logic guards — allow cancel anytime (remove the “cannot cancel after confirm”)
    ...
    // 5) Self-delivery / subscription toggle
    $restaurant    = Helpers::get_restaurant_data();
    $selfDelivery  = (
    ...
    if (
        ...
        && !in_array($order->order_type, ['dine_in','take_away'])
        && !$selfDelivery
    ) {
    ...
    // 7) Deliveryman‐confirmation model guard
    if (
        $request->order_status == 'confirmed'
        && !$selfDelivery
        && config('order_confirmation_model') == 'deliveryman'
        && !in_array($order->order_type, ['take_away','dine_in'])
    ...
    // 8) Delivered → handle payment, OTP, stats
    if ($request->order_status == 'delivered') {
        $verification = (bool)\App\Models\BusinessSetting::where('key','order_delivery_verification')->first()?->value;
        if ($verification) {
            if (!$request->otp || $request->otp != $order->otp) {
                Toastr::warning(translate('messages.order_varification_code_not_matched'));
                return back();
            }
        }
        if (isset($order->subscription_id) && $order->subscription_logs->isEmpty()) {
            Toastr::warning(translate('messages.You_Can_Not_Delivered_This_Subscription_order_Before_Schedule'));
            return back();
        }

        // Ensure transaction exists & mark paid
        if (!$order->transaction || isset($order->subscription_id)) {
            $unpaid     = OrderPayment::where('payment_status','unpaid')
                              ->where('order_id',$order->id)
                              ->first()?->payment_method
                          ?? 'digital_payment';
            $receivedBy = in_array($unpaid, ['cash_on_delivery'])
                          ? 'restaurant'
                          : 'admin';

            $ol = OrderLogic::create_transaction(order:$order, received_by:$receivedBy, status:null);
            if (!$ol) {
        ...
        $order->details->each(fn($item)=> $item->food?->increment('order_count'));
        $order->customer?->increment('order_count');
    }

    // 9) Delivered or Canceled → free up deliveryman
    if (in_array($request->order_status, ['delivered','canceled']) && $order->delivery_man) {
        $dm = $order->delivery_man;
        $dm->current_orders = max(0, $dm->current_orders - 1);
        $dm->save();
    }

    // 10) Canceled → apply reason & counters
    if ($request->order_status == 'canceled') {
        Helpers::increment_order_count($order->restaurant);
        $order->cancellation_reason = $request->reason ?: 'No reason provided';
