...
// 
    ...
    // Create Payment Intent (recommended approach)
    public function createPaymentIntent(Request $request): JsonResponse
    {
        ...
        $amount = in_array($currency_code, $currencies_not_supported_cents) ? (int)$payment_amount : ($payment_amount * 100);
        ...
        try {
            ...
            return response()->json([
                ...
                'publishableKey' => $this->config_values->publishable_key ?? null
            ]);

        } catch (\Exception $exception) {
            return response()->json($this->response_formatter(GATEWAYS_DEFAULT_400, null, ['error' => $exception->getMessage()]), 400);
        }
    }

    // Handle Payment Intent Success
    public function handlePaymentIntentSuccess(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'payment_id' => 'required|uuid',
        ...
        try {
            $paymentIntent = StripePaymentIntent::retrieve($request->payment_intent_id);
            ...
            if ($paymentIntent->status === 'succeeded') {
                ...
                return response()->json($this->response_formatter(GATEWAYS_DEFAULT_200, $data, null), 200);
            }

            return response()->json($this->response_formatter(GATEWAYS_DEFAULT_400, null, ['error' => 'Payment not succeeded']), 400);
    ...
    public function payment_process_3d(Request $request): JsonResponse
    {
        ...
        if (count(json_decode($data['additional_data'],true))>0) {
            ...
            $business_logo = $logo ? dynamicStorage('storage/app/public/business/' . $logo): url('/');
        }
        $currencies_not_supported_cents = ['BIF', 'CLP', 'DJF', 'GNF', 'JPY', 'KMF', 'KRW', 'MGA', 'PYG', 'RWF', 'UGX', 'VND', 'VUV', 'XAF', 'XOF', 'XPF'];
