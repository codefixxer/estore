<div class="content container-fluid initial-38 new-invoice">
    ...
    margin-bottom: .1rem;">
                <div class="text-center">
                    ...
                    <span class="text-muted"> <b>Due At:</b> {{ \Carbon\Carbon::parse($order['schedule_at'])->format('d M Y h:i A') }}</span><br>
                    {{-- <span class="text-muted"> <b>Place At:</b> {{ \Carbon\Carbon::parse($order['created_at'])->format('d M Y h:i A') }}</span> --}}
                </div>
    ...
    margin-bottom: .1rem;">
                    <div class="text-center">
                        <span class="text-muted">@if($order['payment_status'] == 'unpaid') Order Not Paid @else Order has been Paid @endif</span>
                    </div>
                <hr style="border: black dashed 1.3px; margin-top: .1rem;
    margin-bottom: .1rem;">
                                    ...
                                    @if (count(json_decode($detail['variation'], true)) > 0)
                                        ...
                                        @else
                                            @if (isset(json_decode($detail['variation'],true)[0]))
                                                @foreach(json_decode($detail['variation'],true)[0] as $key1 =>$variation)
                                                    <div class="font-size-sm text-body">
                                                        <span style="font-size: 12px;">{{$key1}} :  </span>
                                                        <span class="font-weight-bold" style="font-size: 12px;">{{$variation}}</span>
                                                    </div>
                                                @endforeach
                                            @endif
                                                @break
    ...
    margin-bottom: .1rem;"> --}}
                <div class="d-flex flex-row justify-content-start">
                    <span class="text-capitalize d-flex"><span> @if(translate(str_replace('_', ' ', $order['payment_method'])) == 'Cash on delivery') Paid Cash at Counter  @else Paid with Card @endif</span> </span>
    ...
    @if ($order['order_note'] == null)
                ...
                <div class="p-3">
                    {{-- @if ($order->order_type == 'dine_in' &&  $order?->OrderReference?->table_number )
                        <h5 class="d-flex justify-content-between gap-2">
                            <span class="text-muted"> {{ translate('Table_Number') }} </span>
                            <span class="">{{ $order?->OrderReference?->table_number }}</span>
                        </h5>
                    @endif --}}
                    {{-- @if ($order->order_type == 'dine_in' &&  $order?->OrderReference?->token_number )
                        <h5 class="d-flex justify-content-between gap-2">
                            <span class="text-muted"> {{ translate('Token_Number') }} </span>
                            <span class="">{{ $order?->OrderReference?->token_number }}</span>
                        </h5>
                    @endif --}}
