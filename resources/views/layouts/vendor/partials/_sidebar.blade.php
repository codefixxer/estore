...
<ul class="js-navbar-vertical-aside-submenu nav nav-sub"
    style="display: {{ Request::is('restaurant-panel/order*') && ! Request::is('restaurant-panel/order/subscription*') ? 'block' : 'none' }}">
        ...
        <span class="badge badge-soft-success badge-pill ml-1">
          @if(config('order_confirmation_model') === 'restaurant' || $selfDelivery)
            {{ \App\Models\Order::where([
                 ...
                 'restaurant_id' => App\CentralLogics\Helpers::get_restaurant_id()
               ])->Notpos()
                ->NotDigitalOrder()
                ->hasSubscriptionToday()
                ->OrderScheduledIn(30)
  ...
  <li class="nav-item {{ Request::is('restaurant-panel/order/list/confirmed') ? 'active' : '' }}">
    <a class="nav-link" href="{{ route('vendor.order.list', ['confirmed']) }}">
      ...
      <span class="text-truncate sidebar--badge-container">
        Accepted Orders
        <span class="badge badge-soft-success badge-pill ml-1">
          {{ \App\Models\Order::whereIn('order_status',['confirmed','delivered','handover','cooking'])
  ...
  {{-- Failed Orders --}}
  <li class="nav-item {{ Request::is('restaurant-panel/order/list/failed') ? 'active' : '' }}">
    <a class="nav-link" href="{{ route('vendor.order.list', ['failed']) }}">
      <span class="tio-circle nav-indicator-icon"></span>
      <span class="text-truncate sidebar--badge-container">
        {{ translate('messages.payment_failed') }}
        <span class="badge badge-soft-danger badge-pill ml-1">
          {{ \App\Models\Order::whereNotNull('failed')
               ->where('restaurant_id', App\CentralLogics\Helpers::get_restaurant_id())
               ->Notpos()
               ->count() }}
        </span>
  ...
  {{-- Canceled Orders --}}
  <li class="nav-item {{ Request::is('restaurant-panel/order/list/canceled') ? 'active' : '' }}">
    <a class="nav-link" href="{{ route('vendor.order.list', ['canceled']) }}">
      <span class="tio-circle nav-indicator-icon"></span>
      <span class="text-truncate sidebar--badge-container">
        {{ translate('messages.canceled') }}
        <span class="badge badge-soft-danger badge-pill ml-1">
          {{ \App\Models\Order::where('order_status','canceled')
               ->where('restaurant_id', App\CentralLogics\Helpers::get_restaurant_id())
               ->Notpos()
               ->hasSubscriptionToday()
               ->OrderScheduledIn(30)
